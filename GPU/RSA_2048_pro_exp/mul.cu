...
// 	cudaFuncSetCacheConfig(expmod, cudaFuncCachePreferL1);
	...
	expmod<<<35, 35, 0, stream[0]>>>(d_c, d_a, d_aux, d_pcomp, d_w, d_pinv, d_pplp, d_pc, d_delta, h_nw[0], d_vals1, d_vals2); 
	expmod<<<35, 35, 0, stream[1]>>>(d_c + TESTS * NR, d_b, d_aux + NR, d_pcomp + TESTS * NR * SW, d_w + TW, d_pinv + NR, d_pplp + NR * NR, d_pc + NR, d_delta + NDELTA, h_nw[1], d_vals1 + NR, d_vals2 + NR);
